...
export class FileComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(public fileStorage: FileStorageService, private websocketService: WebsocketService, private usersService: UsersService) {

  }
  ngAfterViewInit(): void {
    this.contentElement = document.getElementById('textArea1') as HTMLTextAreaElement;
  ...
  onKeyDown(index: any, event: any) {
    if(event.key == 'Backspace'){
      ...
      this.insertChar(event.key, index, this.userIndex);
    }
  }
  ...
  insertChar(char: string, index: number, userIndex: number) {
    ...
    if(index <= this.fileStorage.activeFile.tree.nodes.length) {
      let insertedNode: Node = this.fileStorage.activeFile.tree.localInsert(char, index, userIndex);
  ...
  deleteChar(index: number) {
    ...
    if(index - 1 <= this.fileStorage.activeFile.tree.nodes.length) {
      let deletedNodes: Node[] = this.fileStorage.activeFile.tree.localDelete(index - 1);
      this.fileStorage.activeFile.updateContent();
  ...
  remoteInsert(node: Node) {
    if (this.fileStorage.activeFile.tree.remoteInsert(node)) {
      ...
      this.contentElement.setSelectionRange(this.cursorPosition, this.cursorPosition);
